...
int acceptAnswer()
{
	...
	gets(answer);
	if (!answer[0])
	{ // prevent crash if user enters a blank answer
		setCursorPos(logField);
		printf(CLEARFIELD REDFORE "Cannot leave field blank" RESET);
		return 0;
	}
	if (strcmp(answer, "menu") == 0)
	{ // allows the user to open the pause menu
		pauseTimer();
		pauseMenu();
		return -1;
	}
	if (!strchr(answer, ' '))
	{ // prevents crash if answer does not contain a space
		setCursorPos(logField);
...
int cardinalDirection()
{
	...
	return !rowDiff ? 0 : rowDiff == colDiff ? 1
					  : !colDiff			 ? 2
											 : -1;
}
